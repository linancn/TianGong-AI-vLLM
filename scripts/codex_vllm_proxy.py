...
def convert_chat_completion_response(
    ...
    if finish_reason == "length" and not tool_calls:
        ...
        _finalize_tool_response(response_obj, chat_request["messages"], tool_calls, settings, assistant_text=assistant_text)
    else:
        _finalize_text_response(response_obj, chat_request["messages"], assistant_text, settings)
                ...
                if not completed:
                    ...
                    if state.tool_calls:
                        for event in _emit_final_assistant_events(
                            ...
                            yield event
                        _finalize_tool_response(state.response, chat_request["messages"], state.tool_calls, settings, assistant_text=state.assistant_text)
                        ...
                        for event in _emit_final_assistant_events(
                            ...
                            yield event
                        _finalize_text_response(state.response, chat_request["messages"], state.assistant_text, settings)
